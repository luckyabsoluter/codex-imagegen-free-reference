...
Default log names append `.log` to the generated original name. Logs begin with start metadata for endpoint, transport, output path, and request payload. Responses event blocks include a separate UTC `logged_at:` line; Image API JSON/JSONL records use top-level `logged_at` metadata. Image API logs are redacted JSON request/response records; Responses logs keep event structure while redacting image payloads:
...
- The CLI writes `<final-path>.log` next to the Codex-home original. Logs start with endpoint, transport, output path, and request metadata, and each event timestamp is stored as log metadata. Image API logs redact base64 image payloads; Responses logs keep event structure while redacting image payloads.
