...
- For Responses, treat `--model` as an explicit override. When it is omitted, the CLI selects the visible model with the highest cache priority from the selected Codex home's `models_cache.json` and still sends that resolved model because the ChatGPT-account Codex endpoint rejects model-less requests. If the cache is missing or unusable, pass `--model` explicitly or run Codex to refresh its model catalog. The Image API transport continues to default to `gpt-image-2`.
...
- For Codex API direct mode, generated originals and logs are saved under the selected Codex home's `generated_images_free_reference/<YYYY-MM-DD>/`, using the fixed `--timezone` UTC offset when provided or the runtime's local date otherwise. If the date directory cannot be created, save them directly under `generated_images_free_reference/`. The default Responses transport uses the OpenAI SDK and writes append-only redacted request/response event logs; the optional Image API transport writes append-only redacted JSONL event logs. Original filenames use `<uuid>-<name>.<ext>`, resolving `<name>` from explicit `--name`, then a file-form `--copy-to` stem, then the prompt. Logs use `<uuid>-<name>.<ext>.log`. Project-local placement is done by copying the selected original with `--copy-to`. If `--auth-json` is provided, that auth file and its parent directory are used. Otherwise auth discovery checks `$CODEX_HOME/auth.json` first, then `~/.codex/auth.json`.
...
- If the image is only for preview or brainstorming, the file can remain under the selected Codex home's `generated_images_free_reference/<YYYY-MM-DD>/` or its undated fallback directory.
...
- Execution notes such as `Quality:`, `Input fidelity:`, masks, output format, compression, partial images, and output paths belong to the selected execution path. They are supported by the Codex direct CLI and fallback CLI where documented, but they are not built-in `image_gen` tool arguments.
...
- Use `--timezone <offset>` to select the fixed UTC offset used for the dated output directory. Accept `1:30`, `01:00`, `1`, `+01:00`, and `-01:00`; do not use decimal-hour notation such as `1.5`. Keep the offset from `-12:00` through `+14:00`, and omit the option to use the runtime's local date.
- Use `--reasoning-effort <value>` only when the Responses request should explicitly set `reasoning.effort`; omit it to let the resolved model and server defaults apply. Codex's documented default Power setting is currently `gpt-5.6-sol` with medium reasoning. GPT-5.6 Sol supports `none`, `low`, `medium`, `high`, `xhigh`, and `max` through the API; other models can differ, and additional values may become available, so the CLI does not restrict the value. Check each model page and https://developers.openai.com/api/docs/guides/reasoning when selecting an effort.
...
- `--model` explicitly selects the model for the active transport. For Responses, omitting it uses the selected Codex home's model cache as described above. `--image-model` overrides `--model` for Image API calls and maps to the tool-level `model` field for `--transport responses`. `--background transparent` requires a transparency-capable image model and a transparent-capable output format.
