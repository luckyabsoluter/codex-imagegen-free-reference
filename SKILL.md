...
- For Codex API direct mode, generated originals and logs are saved under the selected Codex home's `generated_images_free_reference/<YYYY-MM-DD>/`, using the fixed `--timezone` UTC offset when provided or the runtime's local date otherwise. If the date directory cannot be created, save them directly under `generated_images_free_reference/`. The default Responses transport uses the OpenAI SDK and writes append-only redacted request/response event logs; the optional Image API transport writes append-only redacted JSONL event logs. Original filenames use `<uuid>-<name>.<ext>`, and logs use `<uuid>-<name>.<ext>.log`. Project-local placement is done by copying the selected original with `--copy-to`. If `--auth-json` is provided, that auth file and its parent directory are used. Otherwise auth discovery checks `$CODEX_HOME/auth.json` first, then `~/.codex/auth.json`.
